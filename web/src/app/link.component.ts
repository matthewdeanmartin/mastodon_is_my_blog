  ...
  template: `
    @if (favicons.length > 0) {
      <div class="favicon-row">
        @for (f of favicons; track f.url) {
          @if (!f.error && f.favicon) {
            <a
              [href]="f.url"
              target="_blank"
              ...
              class="favicon-link"
            >
              <img [src]="f.favicon" [alt]="f.domain" class="favicon-img" (error)="onError(f)" />
            </a>
          } @else {
            <a
              [href]="f.url"
              ...
              rel="noopener noreferrer"
              [title]="f.domain"
              class="favicon-link favicon-fallback"
            >
              {{ f.domain.charAt(0).toUpperCase() }}
            </a>
          }
        }
      </div>
...
export class LinkFaviconsComponent implements OnChanges {
  private previewService = inject(LinkPreviewService);
  private cdr = inject(ChangeDetectorRef);

  @Input() urls: string[] = [];

  favicons: { url: string; domain: string; favicon: string | null; error: boolean }[] = [];

  ngOnChanges(): void {
    this.favicons = this.urls.map((url) => {
      let domain = url;
      try {
      ...
      return { url, domain, favicon: null, error: false };
    });

    this.urls.forEach((url, i) => {
      this.previewService.getPreview(url).subscribe({
        next: (preview) => {
          if (this.favicons[i]) {
            this.favicons[i].favicon = preview?.favicon ?? null;
            this.cdr.markForCheck();
          }
        },
        error: () => {
          if (this.favicons[i]) this.favicons[i].error = true;
          this.cdr.markForCheck();
        },
      });
    });
  ...
  onError(f: { url: string; domain: string; favicon: string | null; error: boolean }): void {
    f.error = true;
    this.cdr.markForCheck();
  }
}
