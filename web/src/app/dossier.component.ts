...
@Component({
    ...
    <div class="dossier-page">
      ...
      @if (dossier) {
        ...
        <div class="dossier-header">
          @if (dossier.header) {
            <div class="header-banner" [style.backgroundImage]="'url(' + dossier.header + ')'"></div>
          }
          <div class="header-body">
            ...
            <div class="header-info">
              ...
              @if (dossier.bot) { <span class="bot-badge">BOT</span> }
            </div>
            <div class="header-actions">
              ...
              <button
                ...
                {{ fetchBusy ? 'Starting…' : (catchupStatus?.running ? 'Deep Catch Up Running…' : 'Deep Catch Up') }}
              </button>
        ...
        <div style="margin: 10px 0 18px 0; font-size: 0.84rem; color: #64748b;">
          Deep Catch Up walks this account's full available history in the background so cached dossier stats can fill in over time.
        </div>
        @if (catchupError) {
        ...
        @if (catchupStatus) {
          <div style="margin: 0 0 16px 0; font-size: 0.84rem; color: #475569;">
            @if (catchupStatus.running) {
              <span>{{ catchupStatus.stage }} — {{ catchupStatus.posts_fetched }} posts across {{ catchupStatus.pages_fetched }} page(s)</span>
            } @else {
              <span>Last deep catch-up fetched {{ catchupStatus.posts_fetched }} posts across {{ catchupStatus.pages_fetched }} page(s).</span>
            }
          </div>
        }
        ...
        <!-- Note -->
        @if (dossier.note) {
          <div class="section" [innerHTML]="dossier.note"></div>
        }
            ...
            @for (window of interactionWindows; track window.label) {
              <div class="interaction-cell">
                ...
                <div class="bar-row">
                  ...
                  <div class="bar-track">
                    <div class="bar them" [style.width.%]="barWidth(window.them_to_me, window.max)"></div>
                  </div>
                ...
                <div class="bar-row">
                  ...
                  <div class="bar-track">
                    <div class="bar me" [style.width.%]="barWidth(window.me_to_them, window.max)"></div>
                  </div>
        ...
        @if (dossier.post_reply_ratio !== null) {
          <div class="section">
            ...
            <p class="ratio-value">{{ dossier.post_reply_ratio | number:'1.1-1' }}x more posts than replies</p>
          </div>
        }
          ...
          <div class="section">
            ...
            <div class="hashtag-chips">
              @for (ht of dossier.top_hashtags; track ht.tag) {
                <span class="hashtag-chip">#{{ ht.tag }} <span class="chip-count">{{ ht.count }}</span></span>
              }
            </div>
          </div>
        }
        ...
        <div class="section">
          ...
          <div class="media-grid">
            <div class="media-cell">
              <span class="media-pct">{{ pct(dossier.media_profile.has_media, dossier.media_profile.total) }}%</span>
              <span class="media-label">Images</span>
            </div>
            <div class="media-cell">
              <span class="media-pct">{{ pct(dossier.media_profile.has_video, dossier.media_profile.total) }}%</span>
              <span class="media-label">Video</span>
            </div>
            <div class="media-cell">
              <span class="media-pct">{{ pct(dossier.media_profile.has_link, dossier.media_profile.total) }}%</span>
  ...
  styles: [`
    ...
    .avatar-lg {
      ...
      border: 3px solid white; margin-top: -36px;
    }
    .header-info { flex: 1; }
    .header-info h2 { margin: 0 0 2px 0; font-size: 1.2rem; color: #1f2937; }
    .acct-line { color: #6b7280; margin: 0 0 8px 0; }
    .stat-row { display: flex; gap: 16px; font-size: 0.82rem; color: #374151; }
    .bot-badge {
    ...
    .action-btn.following { background: #dbeafe; border-color: #93c5fd; color: #1e40af; }
    .action-btn.secondary { background: #f9fafb; color: #374151; }
    .section {
      background: white;
      border: 1px solid #e1e8ed;
      border-radius: 8px;
    ...
    .section h4 { margin: 0 0 12px 0; color: #374151; }
    .interaction-grid { display: flex; gap: 16px; flex-wrap: wrap; }
    .interaction-cell {
      flex: 1; min-width: 180px;
      background: #f9fafb; border-radius: 6px; padding: 10px;
    }
    .window-label { font-size: 0.75rem; font-weight: 700; color: #9ca3af; text-transform: uppercase; }
    .bar-row { display: flex; align-items: center; gap: 6px; margin-top: 6px; }
    .bar-label { font-size: 0.72rem; color: #6b7280; width: 70px; flex-shrink: 0; }
    .bar-track { flex: 1; height: 8px; background: #e5e7eb; border-radius: 4px; overflow: hidden; }
    .bar { height: 100%; border-radius: 4px; }
    .bar.them { background: #3b82f6; }
    .bar.me { background: #10b981; }
    .bar-count { font-size: 0.72rem; color: #374151; width: 24px; text-align: right; }
    .ratio-value { color: #374151; margin: 0; }
    .hashtag-chips { display: flex; flex-wrap: wrap; gap: 8px; }
    .hashtag-chip {
      background: #eff6ff; color: #1d4ed8;
      padding: 4px 10px; border-radius: 12px; font-size: 0.82rem;
    }
    .chip-count {
      background: #dbeafe; color: #1e40af;
      padding: 0 5px; border-radius: 6px; font-size: 0.72rem; margin-left: 4px;
    }
    .media-grid { display: flex; gap: 20px; flex-wrap: wrap; }
    .media-cell { text-align: center; }
    .media-pct { display: block; font-size: 1.4rem; font-weight: 700; color: #1f2937; }
    .media-label { font-size: 0.75rem; color: #6b7280; }
  `],
})
export class DossierComponent implements OnInit, OnDestroy {
  ...
  loadDossier(acct: string, identityId: number): void {
    this.loading = true;
    this.error = null;
    this.api.getDossier(acct, identityId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (d) => {
        this.dossier = d;
      ...
      error: (e: unknown) => {
        this.error = 'Failed to load dossier. Try a Deep Fetch from the account page first.';
        this.loading = false;
        console.error(e);
      },
    });
  ...
  deepFetch(): void {
    ...
    this.api.startAccountCatchup(this.dossier.acct, id, 'deep').pipe(takeUntil(this.destroy$)).subscribe({
      next: (status) => {
  ...
  cancelCatchup(): void {
    ...
    this.api.cancelAccountCatchup(this.dossier.acct, id).pipe(takeUntil(this.destroy$)).subscribe({
  ...
  private loadCatchupStatus(acct: string, identityId: number): void {
    this.api.getAccountCatchupStatus(acct, identityId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (status) => {
        this.catchupStatus = status;
