...
async def performance_middleware(request: Request, call_next):
    ...
    try:
        ...
        logger.info(
            f"Request completed: {request.method} {request.url.path} Status: {response.status_code} Time: {elapsed:.3f}s"
        )
        ...
        logger.error(
            f"Request failed: {request.method} {request.url.path} Error: {str(e)} Time: {elapsed:.3f}s"
        )
        raise
