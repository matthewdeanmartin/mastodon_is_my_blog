...
from .token_store import (
    get_token, set_token, init_db,
    CachedPost, async_session, get_last_sync, update_last_sync
)
...
def analyze_content(html: str, media_attachments: list) -> tuple[bool, bool]:
    ...
    if not has_media:
        if soup.find('iframe'): has_video = True

    # Check media types
    for m in media_attachments:
        if m['type'] in ['video', 'gifv']: has_video = True
        if m['type'] == 'image': has_media = True
...
async def sync_timeline(force: bool = False) -> dict:
    ...
    statuses = m.account_statuses(me["id"], limit=40, exclude_reblogs=False, exclude_replies=False)
    ...
    async with async_session() as session:
        for s in statuses:
            ...
            is_reply = s.get('in_reply_to_id') is not None and s.get('in_reply_to_account_id') != me['id']
